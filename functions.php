...
function getCategory($category_id = '', $page = '')
{
    ...
    $html = "<select class='form-control' name='category_id' $fun  id='category_id'>";
...
function getDashboardExpense($type)
{
    ...
    if ($row['price'] > 0) {
        $p = $row['price'];
        $link = "&nbsp;<a class='detail-link' href='dashboard_report.php?from=" . $from . "&to=" . $to . "' target='_blank'>Details</a>";
    }
    return $p . $link;
}
